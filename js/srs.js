...
const App = {
  ...
  fieldConfig: {
    TUVUNG: { front: ["kanji"], back: ["doc", "han_viet", "nghia", "vi_du"] },
    NGUPHAP: { front: ["cautruc"], back: ["nghia", "cau_truc_ngu_phap", "vi_du", "so_sanh_de_nham"] },
  },
  visibleCols: {
    TUVUNG: ["kanji", "doc", "han_viet", "nghia", "vi_du", "status"],
    NGUPHAP: ["cautruc", "nghia", "muc_do", "vi_du", "status"],
  },
  ...
  // Chế độ luyện tốc độ (mojigoi): đếm 30s mỗi câu, đo tổng thời gian làm cả đề
  examSpeedMode: false,
  examPerQTimerHandle: null,
  examPerQSecondsLeft: 30,
  examTotalTimerHandle: null,
  examTotalStartTime: null,
  examQuestionTimeLog: [], // mảng { qIndex, seconds, overTime } ghi lại thời gian mỗi câu để thống kê cuối đề
...
function startExam(examId) {
  ...
  App.examTotalStartTime = null; // reset mốc thời gian cho đề mới (tránh kế thừa từ đề trước)
...
function renderExamQuestion() {
  ...
  // Số câu "làm lại" đang còn trong hàng đợi = tổng hàng đợi - số câu chưa-từng-làm-đúng còn lại
  ...
  // Random thứ tự đáp án mỗi lần hiện
...
function handleExamAnswer(btn, chosenIdx, qIndex, q) {
  ...
  if (App.examSpeedMode && App.examPerQStartedAt) {
    const secondsUsed = (Date.now() - App.examPerQStartedAt) / 1000;
    App.examQuestionTimeLog.push({ qIndex, seconds: secondsUsed, overTime: secondsUsed > 30 });
  }
  ...
  setTimeout(() => {
    // Bỏ câu hiện tại ra khỏi đầu hàng đợi
    App.examQueue.shift();
    ...
    if (correct) {
      ...
      // Sai: đẩy câu này xuống cuối hàng đợi để làm lại sau,
      // không tính vào examAnswered nên không được cộng điểm cho tới khi đúng
...
function renderExamSpeedSummary() {
  ...
  summaryDiv.innerHTML = `
    <div class="exam-speed-summary-title">⚡ Kết quả luyện tốc độ</div>
    <div class="exam-speed-stats-row">
      <div class="exam-speed-stat"><div class="exam-speed-stat-num">${fmtTime(totalSec)}</div><div class="exam-speed-stat-label">tổng thời gian</div></div>
      <div class="exam-speed-stat"><div class="exam-speed-stat-num">${avgSec.toFixed(1)}s</div><div class="exam-speed-stat-label">trung bình/câu</div></div>
      <div class="exam-speed-stat ${overTimeCount > 0 ? "is-warning" : ""}"><div class="exam-speed-stat-num">${overTimeCount}</div><div class="exam-speed-stat-label">câu quá 30s</div></div>
    </div>
  `;
}
...
function renderExamWeaknessSection() {
  ...
  section.innerHTML = `
    <div class="exam-weakness-title">Câu bạn hay sai nhất trong đề này</div>
